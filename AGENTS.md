...
6. Миграции схемы — только аддитивный `ADD COLUMN` в `migrate_schema()` и только после успешного `backup_db()` (fail-safe, иначе abort + `write_log('migrate')`). Бэкапов держать ≤10 (`prune_backups`).
